    ...
    [:tbody
     ...
     [:tr
      ...
      [:th [:select.new-product-input.bluetext.blueborder
            ...
            options]]]]]
   [:div#product-buttons.margin-center
      [:div.big-button.bluetext.blueborder.center-text
...
(defn product-item [product search-or-view]
  ...
  (let [id (if search-or-view
        ...
        style (if search-or-view
                ...
                {:max-height "250px"})]
    ^{:key id}
    [:div.product.margin-center
     {:style style}
     [:div.name-and-category
      [:p.product-name product-name]
      [:p.category-name category-name]]
    (if search-or-view
      [:div.product-view-items
       [:div.product-amount-control.bluetext.blueborder
        {:on-click #(re/dispatch [::events/update-product-amount id true])} "+"]
       [:div.product-amount.bluetext.blueborder amount]
       [:div.product-amount-control.bluetext.blueborder
...
(defn search-view []
  (let [search (str/lower-case @(re/subscribe [::subs/search]))
    ...
    [:div#search-view
     [:div#list-view-top.margin-center
     ...
     [:div#new-product-button.bluetext.blueborder.margin-center
...
(defn list-view []
  (let [active-list @(re/subscribe [::subs/active-list])
    ...
    [:div#list-view
     ...
     [:div#product-list.margin-center product-list]
     [:div#list-controls 
      [:div#pin-button.bluetext.blueborder
       {:on-click #(re/dispatch [::events/toggle-pinned (:id active-list-contents)])}
       pin-text]
      [:div.plus.bluetext.blueborder {:on-click #(re/dispatch [::events/update-search ""])} "+"]]]))
  ...
  (let [active-list @(re/subscribe [::subs/active-list])
    ...
    [:div#container.margin-center
     ...
     (when-not active-list
       [:p.plus.bluetext.blueborder {:on-click #(re/dispatch [::events/init-new-list])} "+"])
     active-component]))
